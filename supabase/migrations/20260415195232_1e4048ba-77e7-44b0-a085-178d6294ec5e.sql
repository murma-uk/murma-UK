...
-- Create enums
CREATE TYPE public.request_category AS ENUM ('opening_hours', 'new_branch', 'classes_sessions', 'artist_visit', 'announcement');
CREATE TYPE public.app_role AS ENUM ('user', 'business', 'authority', 'admin');
...
CREATE POLICY "Authenticated users can create requests" ON public.requests FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Non-flagged comments viewable by everyone" ON public.comments FOR SELECT USING (flagged = false);
...
CREATE TRIGGER on_upvote_insert AFTER INSERT ON public.upvotes FOR EACH ROW EXECUTE FUNCTION public.increment_upvote_count();
CREATE TRIGGER on_upvote_delete AFTER DELETE ON public.upvotes FOR EACH ROW EXECUTE FUNCTION public.decrement_upvote_count();
