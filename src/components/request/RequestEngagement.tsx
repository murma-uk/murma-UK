...
import { Users, Sparkles, GitMerge, Check, X, Loader2 } from "lucide-react";
...
export default function RequestEngagement({ requestId, ownerId }: Props) {
  ...
  const [angles, setAngles] = useState<Comment[]>([]);
  ...
  const load = useCallback(async () => {
    const [{ data: angles_data }, { data: co }, sugs] = await Promise.all([
      supabase
    ...
    const ang = (angles_data ?? []) as Comment[];
